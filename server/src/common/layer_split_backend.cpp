...
GenerateResult LayerSplitBackend::restore_chain_and_generate_impl(
    ...
    if (!adapter_ || !adapter_->apply_restore_chain(thick_slot, thin_slots)) {
        result.fail(GenerateErrorCode::BackendSpecific, "restore_chain");
        io.emit(-1);
        return result;
    }
    ...
    if ((int)req.prompt.size() > snap_pos) {
        ...
        GenerateResult gen = run_from_state(
            delta_req, io, snap_pos, /*reset_state=*/false, req.prompt);
    ...
    GenerateResult gen = run_from_state(
        req, io, snap_pos, /*reset_state=*/false, req.prompt);
    gen.restore_s = result.restore_s;
    gen.suffix_n = result.suffix_n;
    std::fprintf(stderr,
        "[restore-chain] thick=%d thin_n=%zu snap_pos=%d restore_s=%.3f "
